...
class Environment:
    ...
    def __init__(self, agent):        
        ...
        self.goal_index = random.randint(0, len(self.obstacles))
    ...
    def run(self):
        ...
        predicted_path_to_draw = self.agent.planning(self.goal_index, self.obstacles)
        # Visualization
        ...
        if round(dist_to_goal, 3) < self.agent.ROBOT_RADIUS + self.OBSTACLE_RADIUS:
            # self.goal_flag = True
            print('Arrive at the goal #{}, Simulation finished!'.format(self.goal_index))
            # Choose a new goal
            self.goal_index = random.randint(0, len(self.obstacles))
            print('Resetting new goal #{}'.format(self.goal_index))
...
class NewDWA:
    ...
    def planning(self, goal_index, obstacles):
        """ Planning trajectory and select linear and angular velocities for robot
            ...
            @param goal_index:  goal_index in obstacles
        ...
        for vLpossible in vLpossiblearray:
            for vRpossible in vRpossiblearray:
                ...
                if vLpossible <= self.MAX_VEL_LINEAR and vRpossible <= self.MAX_VEL_ANGULAR and vLpossible >= 0 and vRpossible >= -self.MAX_VEL_ANGULAR:
                    ...
                    distanceToObstacle = self.calculateClosestObstacleDistance(predict_x, predict_y, goal_index, obstacles)
